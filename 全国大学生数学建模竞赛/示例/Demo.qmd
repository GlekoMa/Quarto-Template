...
abstract: |-
  ...
  模型建立方面，本文采用了两种标准化方法与三种客观赋权法，相应总共建立了五个模型（$2\times 3 =
  6$，本应是六个模型，但经检验，其中一个模型所采用的标准化方法与所采用的加权法不相容，故将其去除），紧接以理想解法分别求出了其相应院校排名，然后计算出各个院校在五次排名结果中的平均得分，最后以这个平均得分的排名作为本文的最终评估结果。
keyword: 相关分析 \ 分布拟合 \ 标准化 \ 客观赋权法 \ 理想解法
---
...
为客观评价我国研究生教育实际状况与各研究生院教学质量，需要先抽取小样本进行试评估。所收集的数据如 @tbl-试评估数据

```{r}
#| echo: false
#| message: false
#| label: tbl-试评估数据
#| tbl-cap: "研究生院的试评估数据资料"
#| fig-pos: H
library(dplyr)
library(knitr)
library(kableExtra)
df0_unit = df0
names(df0_unit) = c("人均专著（本/人）", "生师比", "科研经费（万元/年）", "逾期毕业率")
kbl(df0_unit, booktabs = T) %>%
...
本文首先对数据中各属性指标间的相关性进行了可视化，如 @fig-原数据相关性 。
...
剔除第五个院校的数据后留存的数据(单位已略去)如 @tbl-剔除后数据 .
...
#| tbl-cap: "剔除第五所院校后的数据"
df0 = df0[-5, ]
kbl(df0, booktabs = T, position = "H") %>%
  kable_styling(latex_options = "striped")
```

再次对该数据的相关性进行可视化，结果如 @fig-剔除后相关性 .
...
可以看出在剔除第五行数据后，生师比与人均专著展现出了极高的相关性。为探究其相关关系，本文对这两个属性指标进行了线性回归拟合，可视化结果如 @fig-回归拟合 .
...
在所给数据当中，人均专著和科研经费显然是效益型属性，即相同条件下，它们的值越大，代表该院校实力越强，而逾期毕业率则显然是成本型属性，在院校的评估中，其值越小越好。而生师比这一属性指标无法简单直接地看出它对院校评估所应起到的影响，本节的第一部分就包含了对其的探讨与相应值的转换。
...
查阅相关资料\[1\]，国际一流大学的生师比所在区间一般为2到4，就目前所剩数据（剔除第五行后）而言，无一达到。而该资料\[1\]也表明，生师比在10以内都可以接受。资料\[1\]中有世界一流院校的生师比数据（七十四个）统计情况如 @fig-生师比资料图 .

![一流大学研究生生师比分类统计图示](SSB.jpg){#fig-生师比资料图 fig.pos="H"}

可以看出（本文只参考了统计图中公立院校的数据），一流大学的生师比集中在2左右，为偏态分布。为确定本文模型生师比加权系数，本文根据参考资料对世界一流大学的调查统计图进行了数据复现并对其进行了伽马分布拟合，拟合结果其基本服从$X\sim Ga(2.256,0.797)$，经检验，其P值小于0.01，通过显著性检验。拟合效果如 @fig-伽马分布拟合 .
...
df_ga = data.frame(
  x = rgamma(1000000, shape = fitt$estimate[1], rate = fitt$estimate[2])
)
df_ssb = data.frame(
  x = ssb
)

ks = ks.test(jitter(ssb, 0.001), "pgamma", shape = 0.347, rate = 0.137)
...
本文所采用的两种标准化方法与对应公式分别为（注：若不做说明，$i=1,\cdots,n \quad j=1,\cdots,m$对以下所有公式皆适用）：
...
由min-max法标准化公式([-@eq-a])（注：由本数据为4行4列，故以下公式中，皆有$i=1,2,3,4, \quad j=1,2,3,4, \quad n=4, \quad m=4$）：

$$
b_{i j} = \frac{a_{i j}-\min a_{i j}}{\max a_{i j}-\min a_{i j}}.
$$

可得标准化后的数据如 @tbl-模型一标准化后数据 .
...
df0_a = ax(df0)
kbl(df0_a, booktabs = T, position = "H") %>%
  kable_styling(latex_options = "striped")
```

由变异系数法加权公式([-@eq-d])([-@eq-e])([-@eq-k])：
...
可得加权后的数据如 @tbl-模型一标准化并加权后数据 .

```{r}
#| echo: false
#| message: false
#| label: tbl-模型一标准化并加权后数据
#| tbl-cap: 模型一标准化并加权后的数据
df0_aa = xa(df0_a)
kbl(df0_aa, booktabs = T, position = "H") %>% 
  kable_styling(latex_options = "striped")
```
...
由正负理想解带入相应公式得到的数据如 @tbl-模型一理想解表 .
...
#| label: tbl-模型一理想解表
#| tbl-cap: "模型一理想解表"
kbl(over(df0_aa), booktabs = T, position = "H", escape = F) %>% 
  kable_styling(latex_options = "striped")
```
...
现直接给出模型二最终相关数据,如 @tbl-模型二最终相关数据 .
...
kbl(df_ab, booktabs = T, position = "H", escape = F) %>% 
...
现直接给出模型三最终相关数据,如 @tbl-模型三最终相关数据 .
...
kbl(df_ac, booktabs = T, position = "H", escape = F) %>% 
  add_header_above(c(" ", "处理后的数据" = 4, "理想解" = 3)) %>% 
  kable_styling(latex_options = "striped")
```
...
现直接给出模型四最终相关数据,如 @tbl-模型四最终相关数据 .
...
kbl(df_ba, booktabs = T, position = "H", escape = F) %>% 
  add_header_above(c(" ", "处理后的数据" = 4, "理想解" = 3)) %>% 
  kable_styling(latex_options = "striped")
```
...
现直接给出模型五最终相关数据,如 @tbl-模型五最终相关数据 .
...
kbl(df_bb, booktabs = T, position = "H", escape = F) %>% 
  add_header_above(c(" ", "处理后的数据" = 4, "理想解" = 3)) %>% 
  kable_styling(latex_options = "striped")
```
...
kbl(df_rank, booktabs = T, position = "H", escape = F) %>% 
  kable_styling(latex_options = "striped")
```

假设第一至四名分别对应得分为4、3、2、1，则它们的最终总得分排名如 @tbl-总得分排名 .
...
kbl(df_point, booktabs = T, position = "H", escape = F) %>% 
  kable_styling(latex_options = "striped")
```
